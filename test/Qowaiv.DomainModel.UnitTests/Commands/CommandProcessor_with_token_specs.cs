...
namespace Commands.CommandProcessor_with_token_specs
{
    public class Sends
    {
        [Test]
        public async Task a_command()
        {
            ...
            (await processor.Send(new One(), token: default)).Value.Should().Be(1);
        }
    ...
    public class Throws
    {
        ...
        {
            Action send = () => new NumberProcessor(new NumberHandler()).Send(null, token: default);
        ...
        {
            Action send = () => new NumberProcessor(null).Send(new One(), token: default);
            send.Should().Throw<UnresolvedCommandHandler>()
                .WithMessage("The command handler Commands.CommandProcessor_with_token_specs.CommandHandler`1[Commands.CommandProcessor_with_token_specs.One] could not be resolved.");
        }
    }
    ...
    {
        Task<Result<int>> Handle(TCommand command, CancellationToken token);
    }
    ...
    class NumberHandler :
        ...
        CommandHandler<Two>
    {
        public Task<Result<int>> Handle(One command, CancellationToken token) => Result.For(1).AsTask();
        public Task<Result<int>> Handle(Two command, CancellationToken token) => Result.For(2).AsTask();
    }
}
